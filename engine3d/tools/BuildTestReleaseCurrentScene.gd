...
const DEFAULT_OUT_DIR: String = "D:/Test/current_scene"
const DEFAULT_POWERSHELL: String = "powershell.exe"
const DEFAULT_PRESET_NAME: String = "Windows Desktop"
const DEFAULT_RELEASE_TAG: String = "test-build-current-scene"
const DEFAULT_RELEASE_TITLE: String = "Test Build - Current Scene"
...
func _parse_args(args: PackedStringArray) -> Dictionary:
	...
	for arg: String in args:
		...
		elif arg.begins_with("--current-scene="):
			cfg["current_scene"] = EXPORT_HELPER.strip_wrapped_quotes(arg.substr(16))
...
func _print_usage() -> void:
	print("BuildTestReleaseCurrentScene (Godot CLI)")
	print("Builds a runnable Windows test release (exe + pck) with an overridden startup scene.")


func _run(cfg: Dictionary) -> int:
	var out_dir: String = str(cfg.get("out_dir", DEFAULT_OUT_DIR)).strip_edges()
	var current_scene: String = str(cfg.get("current_scene", "")).strip_edges()
	...
	var summary_abs: String = out_dir_abs.path_join("current_scene_summary.txt")
	var preflight_summary_abs: String = out_dir_abs.path_join("current_scene_preflight_summary.txt")
	var manifest_report_abs: String = out_dir_abs.path_join("current_scene_runtime_manifest_report.json")

	if not _is_supported_output_path(out_dir):
		push_error("BuildTestReleaseCurrentScene: out must be a res://, user://, or absolute path.")
		return 1
	if current_scene.is_empty() or not current_scene.begins_with("res://") or not ResourceLoader.exists(current_scene):
		push_error("BuildTestReleaseCurrentScene: current-scene is missing or invalid: %s" % current_scene)
		return 1
	var manifest_scan: Dictionary = RUNTIME_MANIFEST.collect_runtime_paths(PackedStringArray([current_scene]), runtime_manifest_path)
	if not bool(manifest_scan.get("ok", false)):
		push_error("BuildTestReleaseCurrentScene: runtime manifest failed: %s" % str(manifest_scan.get("message", "unknown error")))
		return 1
	...
	if godot_exe.is_empty() or not FileAccess.file_exists(godot_exe):
		push_error("BuildTestReleaseCurrentScene: could not resolve Godot executable path.")
		return 1
	var templates: Dictionary = EXPORT_HELPER.find_windows_export_templates()
	if not bool(templates.get("ok", false)):
		push_error("BuildTestReleaseCurrentScene: %s" % str(templates.get("message", "Missing export templates.")))
		return 1
	...
	if not missing_autoloads.is_empty():
		push_error("BuildTestReleaseCurrentScene: missing autoload paths: %s" % ", ".join(missing_autoloads))
		return 1
	...
	if preflight_only:
		...
		print("BuildTestReleaseCurrentScene: preflight ok include_count=%d missing_count=%d" % [
	...
	var project_file_abs: String = project_root_abs.path_join("project.godot")
	var export_presets_abs: String = project_root_abs.path_join("export_presets.cfg")
	var project_original: String = FileAccess.get_file_as_string(project_file_abs)
	var export_presets_original: String = FileAccess.get_file_as_string(export_presets_abs)
	if project_original.is_empty() or export_presets_original.is_empty():
		push_error("BuildTestReleaseCurrentScene: project.godot or export_presets.cfg is missing or unreadable.")
		return 1
	var project_patched: String = EXPORT_HELPER.override_project_main_scene_text(project_original, current_scene)
	var export_presets_patched: String = EXPORT_HELPER.set_embed_pck_text(export_presets_original, false)
	if use_runtime_manifest:
		export_presets_patched = RUNTIME_MANIFEST.apply_manifest_to_export_preset_text(export_presets_patched, PackedStringArray([current_scene]), manifest_scan)
	if not _write_text_file(project_file_abs, project_patched):
		return 1
	if not _write_text_file(export_presets_abs, export_presets_patched):
		_write_text_file(project_file_abs, project_original)
		return 1

	var export_ok: bool = _export_release(godot_exe, project_root_abs, exe_abs)
	var restore_project_ok: bool = _write_text_file(project_file_abs, project_original)
	var restore_export_ok: bool = _write_text_file(export_presets_abs, export_presets_original)
	if not restore_project_ok or not restore_export_ok:
		push_error("BuildTestReleaseCurrentScene: failed to restore project configuration after export.")
		return 1
	if not export_ok:
		return 1
	if not FileAccess.file_exists(exe_abs):
		push_error("BuildTestReleaseCurrentScene: export did not produce executable: %s" % exe_abs)
		return 1
	if not FileAccess.file_exists(pck_abs):
		push_error("BuildTestReleaseCurrentScene: export did not produce data package: %s" % pck_abs)
		return 1
	...
	if zip_output:
		...
		if not FileAccess.file_exists(zip_abs):
			push_error("BuildTestReleaseCurrentScene: expected zip missing after compression: %s" % zip_abs)
			return 1
	if publish_requested:
		publish_result = _publish(zip_abs, cfg, project_name, current_scene)
	...
	print("BuildTestReleaseCurrentScene: success exe=%s pck=%s" % [exe_abs, pck_abs])
	return 0


func _export_release(godot_exe: String, project_root_abs: String, exe_abs: String) -> bool:
	return _exec_ok(godot_exe, PackedStringArray(["--headless", "--path", project_root_abs, "--export-release", DEFAULT_PRESET_NAME, exe_abs]), "BuildTestReleaseCurrentScene")


func _publish(zip_abs: String, cfg: Dictionary, project_name: String, current_scene: String) -> Dictionary:
	if not FileAccess.file_exists(zip_abs):
	...
	var result: Dictionary = GITEA_PUBLISHER.publish_release_asset({
		...
		"body": "\n".join(body_lines),
		"asset_path": zip_abs,
		"asset_name": "%s-test-current-scene.zip" % project_name,
...
func _zip_directory(source_dir_abs: String, zip_abs: String, powershell_exe: String) -> bool:
	...
	var tar_result: bool = _exec_ok("tar.exe", PackedStringArray(["-a", "-cf", zip_abs, "-C", source_dir_abs, "."]), "BuildTestReleaseCurrentScene")
	...
	return _exec_ok(powershell_exe, PackedStringArray(["-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", ps_script]), "BuildTestReleaseCurrentScene")


func _reset_output_dir(out_dir_abs: String, powershell_exe: String) -> bool:
	if powershell_exe.is_empty():
		push_error("BuildTestReleaseCurrentScene: powershell executable is empty.")
		return false
...
func _write_text_file(abs_path: String, text: String) -> bool:
	...
	if f == null:
		push_error("BuildTestReleaseCurrentScene: failed to write %s" % abs_path)
		return false
	f.store_string(text)
	f.close()
	return true
